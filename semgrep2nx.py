...
all = {
    ...
    "metavariable-name": [["Filter","\"metavariable-name\""]],
...
def Semgrep2NX(expl, C):
    ...
    if is_leaf(expl):
        ...
        G.add_edge(st, c)
        G.remove_edge(st, ed)
    elif not expl['sanitizer'] and is_and(expl) or expl['sanitizer'] and is_or(expl):
        result = []
        next_connect = st
    ...
    elif not expl['sanitizer'] and is_or(expl) or expl['sanitizer'] and is_and(expl):
...
def is_true(G, node):
    ...
    expl = nx.get_node_attributes(G, 'fact')[node]

    if label == "Start" or label == "End":
        return True
    matched = nx.get_node_attributes(G, 'fact')[node]['matches'] != []
    if expl['sanitizer']:
        return not matched
    return matched
